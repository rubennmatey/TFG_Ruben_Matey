...
# Signs and sends a log batch hash transaction to the blockchain
def send_log_batch_to_blockchain(batch_hash, timestamp):
    web3, contract = get_contract()

    account = Web3.to_checksum_address(BLOCKCHAIN_ACCOUNT)
    nonce = web3.eth.get_transaction_count(account)

    transaction = contract.functions.registerLogBatch(
        batch_hash,
        timestamp
    ).build_transaction({
        "chainId": CHAIN_ID,
